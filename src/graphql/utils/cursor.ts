...
export function createEdgesWithPageInfo ({ data, totalCount, idFirstRecord }, { first = Infinity, sortBy = "_id" }): ReturnEdges {
  ...
  if (totalCount === 0) {
    ...
    hasPreviousPage = idFirstRecord?.toString() !== data?.[0]?._id?.toString();
  }
