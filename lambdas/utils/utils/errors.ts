...
export class DomainError extends Error {
  ...
  constructor(message: string, code = ERROR_CODES.UNKNOWN) {
    super(message);
...
export function assert(predicate: any, message: string, code: string) {
  if (predicate) return;
  throw new DomainError(message, code);
}

export function throwError(message: string, code: string) {
  throw new DomainError(message, code);
}
