...
export const user: Resolvers<Context> = {
  ...
  Query: {
    async me(_, __, ctx) {
      const claim = JSON.parse(
        decodeURIComponent(atob(ctx.headers?.accesstoken))
      );
  ...
  Mutation: {
    async signUp(_, args, ctx) {
      let tenant = await ctx.dataSources.db.tenant({
        where: { name: { _eq: "default" } },
      });
      if (!tenant) {
      ...
      await ctx?.dataSources?.cognito?.updateCognitoUserAttributes(
        args.input?.email.toLowerCase(),
        updatedAttributes
      );
      await ctx?.dataSources?.cognito?.confirmSignUp(
        args.input?.email.toLowerCase()
      );
    ...
    async createUser(_, args, ctx) {
      const user = await ctx.dataSources.db.user({
        where: { email: { _eq: args?.input?.email.toLowerCase() } },
      });
      ...
      if (args?.input?.tenantId) {
        tenant = await ctx.dataSources.db.tenant({
          where: { _id: { _eq: args?.input?.tenantId } },
        });
      } else {
        tenant = await ctx.dataSources.db.tenant({
          where: { name: { _eq: "default" } },
      ...
      const cognitoUser: any =
        await ctx?.dataSources?.cognito?.createCognitoUser(newRecord);
      assert(
        cognitoUser,
        ERROR_MESSAGES.COGNITO_EMAIL_ALREADY_EXISTS,
    ...
    async updateUser(_, args, ctx) {
      ...
      if (typeof args?.input?.mfa === "boolean") {
        await ctx?.dataSources?.cognito?.setCognitoUserMFAPreference(
          updatedUser.email,
          updatedUser?.mfa
        );
      }
